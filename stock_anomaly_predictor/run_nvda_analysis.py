#!/usr/bin/env python3
"""
Run Full Analysis for NVDA (Nvidia)
Test system predictability on a high-momentum tech stock
"""
...
print("=" * 70)
print("STOCK ANOMALY PREDICTION SYSTEM - NVDA ANALYSIS")
print("Testing system predictability on Nvidia")
print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("=" * 70)

# Update config for NVDA
from config.settings import config
config.data.ticker = "NVDA"
config.data.start_date = "2021-08-25"  # Same timeframe as RKLB

# Update related stocks for semiconductor/AI sector
config.data.related_tickers = [
    ...
    "MSFT",      # Microsoft (AI partner)
    "GOOGL",     # Google (AI competitor)
]
...
# ============================================================
print("\n" + "=" * 70)
print("SIGNAL PERIODS ANALYSIS - NVDA")
print("=" * 70)
...
if len(signal_dates) > 0:
    ...
    # Print each signal
    print(f"\n{'Date':<12} {'Price':>10} {'Score':>7} {'1D':>8} {'5D':>8} {'10D':>8} {'20D':>8} {'Result'}")
    print("-" * 85)
    
    for _, row in signals_df.iterrows():
        date_str = row['date'].strftime('%Y-%m-%d')
        result = "✓ WIN" if row['fwd_10d'] > 0 else "✗ LOSS" if not np.isnan(row['fwd_10d']) else "PENDING"
        print(f"{date_str:<12} ${row['price']:>9.2f} {row['composite_score']:>6.1f} "
    ...
    # Identify signals that caught major AI rallies
    print("\n" + "-" * 70)
    print("SIGNALS DURING KEY NVDA EVENTS")
    print("-" * 70)
    
    # Check for signals around major NVDA dates
    key_events = [
        ("2023-05-24", "ChatGPT-driven earnings blowout"),
        ("2023-08-23", "Q2 2023 earnings beat"),
        ("2024-02-21", "Q4 2023 AI demand surge"),
        ("2024-05-22", "Q1 2024 earnings + stock split"),
        ("2024-08-28", "Q2 2024 earnings"),
    ]
    
    # Convert signals_df date column to tz-naive for comparison
    signals_df_copy = signals_df.copy()
    if hasattr(signals_df_copy['date'].iloc[0], 'tz'):
        signals_df_copy['date'] = signals_df_copy['date'].dt.tz_localize(None)
    
    print("\nKey NVDA Events and Signal Detection:")
    for event_date, description in key_events:
        event_dt = pd.Timestamp(event_date)
        # Check if there was a signal within 10 days before the event
        nearby_signals = signals_df_copy[
            (signals_df_copy['date'] >= event_dt - pd.Timedelta(days=10)) &
            (signals_df_copy['date'] <= event_dt)
        ]
        if len(nearby_signals) > 0:
            for _, sig in nearby_signals.iterrows():
                print(f"  ✓ {sig['date'].strftime('%Y-%m-%d')}: Signal detected before {description}")
                print(f"    → 10-day return: {sig['fwd_10d']:+.1f}%")
        else:
            print(f"  ✗ No signal before {event_date}: {description}")
    
    # Save signal details
    signals_df.to_csv('output/nvda_signals.csv', index=False)
    print(f"\n✓ Signal details saved to output/nvda_signals.csv")

else:
    print("\nNo actionable signals generated for NVDA in this period.")
...
try:
    ...
    report = dashboard.generate_summary_report(
        backtest_result,
        analysis_summary,
        save_path='output/nvda_report.txt'
    )
    ...
    try:
        ...
        dashboard.plot_price_with_signals(
            ...
            save_path='output/nvda_price_signals.png'
        )
        
        dashboard.plot_anomaly_analysis(
            primary_df, stat_anomalies,
            save_path='output/nvda_anomaly_analysis.png'
        )
...
# Final Summary
print("\n" + "=" * 70)
print("FINAL ANALYSIS SUMMARY - NVDA")
print("=" * 70)
...
Total Period Return: {total_return:+.1f}%
...
# COMPARISON TABLE
print("=" * 70)
print("COMPARISON: NVDA vs RKLB vs BABA")
print("=" * 70)
print("""
Metric                    NVDA            RKLB            BABA
--------------------------------------------------------------""")
print(f"Hurst Exponent            {hurst:.3f}           1.040           0.996")
print(f"Beta                      {market_summary.get('beta', 0):.2f}            4.52            0.69")
print(f"Total Signals             {len(signal_dates)}              46              52")
print(f"Backtest Win Rate         {backtest_result.win_rate:.1%}           47.8%           42.3%")
print(f"Profit Factor             {backtest_result.profit_factor:.2f}            1.96            1.04")

if len(valid_10d) > 0:
    print(f"10-Day Win Rate           {valid_10d['win_10d'].mean():.1%}           69.6%           51.9%")
    print(f"Avg 10-Day Return         {valid_10d['fwd_10d'].mean():+.2f}%          ~2.0%          +0.82%")
...
with open('output/nvda_results.pkl', 'wb') as f:
    pickle.dump(results, f)
print("\nResults saved to output/nvda_results.pkl")
